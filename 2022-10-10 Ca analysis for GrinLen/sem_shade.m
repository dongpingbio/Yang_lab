function [lineOut, fillOut, amean, asem] = sem_shade(amatrix,alpha,acolor,F,smth)
...
asem = astd/sqrt (size (amatrix,1));

% astd = nanstd(amatrix,[],1)/sqrt(size(amatrix,1)); % to get sem shading
if exist('alpha','var')==0 || isempty(alpha) 
    fillOut = fill([F fliplr(F)],[amean+asem fliplr(amean-asem)],acolor,'linestyle','none');
    acolor='k';
else
    fillOut = fill([F fliplr(F)],[amean+asem fliplr(amean-asem)],acolor, 'FaceAlpha', alpha,'linestyle','none');
end
if ishold==0
